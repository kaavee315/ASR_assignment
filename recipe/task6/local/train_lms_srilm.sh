...
echo "------------------------"
ngram-count -lm $tgtdir/2gram.kn01.gz -"$discount"1 -gt1min 0 -"$discount"2 -gt2min 1 -order "$order" -text $tgtdir/train.txt -vocab $tgtdir/vocab -unk -sort -map-unk "$oov_symbol"
# ngram-count -lm $tgtdir/2gram.kn01.gz -"$discount"1 -gt1min 0 -"$discount"2 -gt2min 1 -"$discount"3 -gt3min 1 -order "$order" -text $tgtdir/train.txt -vocab $tgtdir/vocab -unk -sort -map-unk "$oov_symbol"
...
echo "--------------------"
(
  for f in $tgtdir/2gram* ; do ( echo $f; ngram -order $order -lm $f -unk -map-unk "$oov_symbol" -ppl $tgtdir/dev.txt ) | paste -s -d ' ' - ; done
